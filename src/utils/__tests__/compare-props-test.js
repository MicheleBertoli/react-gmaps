...
describe('compareProps', () => {
  ...
  it('returns false if the objects have different keys', () => {
    const result = compareProps({ a: 1 }, { b: 1 });
    expect(result).toBe(false);
  });
  ...
  it('returns false if nested objects have different keys', () => {
    const result = compareProps({ a: { b: 1 } }, { a: { c: 1 } });
    expect(result).toBe(false);
  });

  it('returns false if nested objects have different values', () => {
    const result = compareProps({ a: { b: 1 } }, { a: { b: 2 } });
    expect(result).toBe(false);
  });

  it('returns true if nested objects have the same keys/values', () => {
    const result = compareProps({ a: { b: 1 } }, { a: { b: 1 } });
    expect(result).toBe(true);
  });
